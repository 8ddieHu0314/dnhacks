...
- **proto-shield**: no datasheet or manufacturer source
...
- **ir-remote**: no datasheet or manufacturer source
...
- **uno-r3**: exact USB-to-serial chip and MCU package on this specific ELEGOO kit bundle revision (CH340C+SMD confirmed for the standalone board vs ATMEGA16U2+DIP-28 seen on an Amazon listing of the same product name), which specific digital pins are PWM-capable (listed here as D3, D5, D6, D9, D10, D11 by standard Arduino Uno Rev3 convention; only the count of 6 was confirmed via a fetched source, not the individual pin numbers), board dimensions and weight are the official Arduino Uno Rev3 reference numbers, not independently confirmed for the ELEGOO clone, 5V/3.3V rail and per-pin current limits are official Arduino Uno Rev3 numbers, assumed to apply to the ELEGOO clone but not independently verified
...
- **proto-shield**: pins.pinout (no separate pin-by-pin table confirmed; the shield is understood to mirror the Uno's own header 1:1 based on product descriptions, but this was not verified against a diagram, so pin_count is left at 0 and pinout empty rather than guessed), electrical.absolute_max_voltage and max_current (the shield has no rating of its own beyond the Uno underneath it), exact manufacturer / part number (ELEGOO does not sell this piece as a standalone SKU, so pricing and specs are sourced from a generic equivalent rather than an ELEGOO-branded listing), no manufacturer or datasheet-type source exists for this generic passive tool; both cited sources are a tutorial and a retailer listing
...
- **ir-remote**: identity.manufacturer_part_number, electrical.operating_voltage (generic reference value, not re-measured for the exact ELEGOO unit), electrical.absolute_max_voltage, electrical.typical_current, electrical.max_current, key_specs.key_count (multiple unfetched search snippets say 21 keys for the exact ELEGOO remote, consistent with the seed, but no source that loaded this session states the ELEGOO-specific key count; the one fetched retailer source describing a similar kit has 18 keys, a different remote), key_specs.battery_model (generic reference value, not re-confirmed for the exact ELEGOO unit), key_specs.claimed_range (generic reference value, not re-confirmed for the exact ELEGOO unit), visual_identification.color_and_markings, market.unit_price_usd, market.typical_suppliers
...
- **rtc-ds3231**: identity.manufacturer_part_number: the exact RTC chip populated on kits currently shipping. ELEGOO's own tutorial text says DS1307, but its own wiring instructions describe a 32K pin the real DS1307 chip does not have per its datasheet; that pin is standard on DS3231-based boards, which share the same generic 6-pin PCB layout. Not resolved by any fetched source., key_specs.backup_battery cell size for the exact kit-bundled module (CR1220 confirmed only for a separately sold ELEGOO spare-part listing, not the kit tutorial itself), electrical absolute maximum ratings and current figures are for the bare DS1307 chip only; module-level figures (with any onboard regulation) are not confirmed, visual_identification.color_and_markings
...
|---|---|---|---|---|
| uno-r3 | verified_with_gaps | medium | 5 | 13.55 |
| lcd1602 | verified_with_gaps | medium | 3 | 9.95 |
| proto-shield | verified_with_gaps | low | 2 | 3.99 |
...
| ir-remote | verified_with_gaps | low | 2 | 2.0 |
...
| rtc-ds3231 | verified_with_gaps | low | 4 | 1.04 |
