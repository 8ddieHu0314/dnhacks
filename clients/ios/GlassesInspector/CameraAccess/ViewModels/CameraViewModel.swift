...
final class CameraViewModel {
  ...
  private func beginStream(on session: DeviceSession) {
    ...
    let resolution: StreamingResolution = {
      switch UserDefaults.standard.string(forKey: "streamResolution") ?? "high" {
      case "high": return .high
      case "low": return .low
      default: return .medium
      }
    }()
    let requestedFPS = UInt(UserDefaults.standard.integer(forKey: "streamFPS"))
    let config = StreamConfiguration(
      videoCodec: VideoCodec.hvc1,
      resolution: resolution,
      frameRate: [15, 24, 30].contains(requestedFPS) ? requestedFPS : 24
    )
